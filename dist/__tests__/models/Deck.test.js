...
describe("Deck", () => {
    ...
    describe("initialization", () => {
        it("should create a standard 52-card deck", () => {
            ...
            expect(suits.has(types_1.Suit.CLUBS)).toBe(true);
            expect(suits.has(types_1.Suit.DIAMONDS)).toBe(true);
            expect(suits.has(types_1.Suit.HEARTS)).toBe(true);
            expect(suits.has(types_1.Suit.SPADES)).toBe(true);
            // Check for all ranks
            const ranks = new Set(cards.map((card) => card.rank));
            expect(ranks.size).toBe(13);
            expect(ranks.has(types_1.Rank.ACE)).toBe(true);
            expect(ranks.has(types_1.Rank.KING)).toBe(true);
            expect(ranks.has(types_1.Rank.QUEEN)).toBe(true);
            expect(ranks.has(types_1.Rank.JACK)).toBe(true);
            expect(ranks.has(types_1.Rank.TEN)).toBe(true);
            expect(ranks.has(types_1.Rank.NINE)).toBe(true);
            expect(ranks.has(types_1.Rank.EIGHT)).toBe(true);
            expect(ranks.has(types_1.Rank.SEVEN)).toBe(true);
            expect(ranks.has(types_1.Rank.SIX)).toBe(true);
            expect(ranks.has(types_1.Rank.FIVE)).toBe(true);
            expect(ranks.has(types_1.Rank.FOUR)).toBe(true);
            expect(ranks.has(types_1.Rank.THREE)).toBe(true);
            expect(ranks.has(types_1.Rank.TWO)).toBe(true);
            // Check that each card is unique
            const uniqueCards = new Set(cards.map((card) => `${card.suit}-${card.rank}`));
