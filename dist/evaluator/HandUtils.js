...
class HandUtils {
     ...
     */
    static sortByRank(cards, aceHigh = true) {
     ...
     */
    static findThreeOfAKinds(cards) {
        const groups = this.groupByRank(cards);
        const trips = [];
        for (const [_, sameRankCards] of groups) {
     ...
     */
    static findStraight(cards, aceHigh = true) {
        if (cards.length < 5) {
            return null;
        }
        // Sort cards by rank
        ...
        const highestOfRank = new Map();
        for (const card of sortedCards) {
            const value = card.getValue(aceHigh);
            if (!highestOfRank.has(value) ||
     ...
     */
    static findStraightFlush(cards, aceHigh = true) {
        if (cards.length < 5) {
            return null;
        }
        const suitGroups = this.groupBySuit(cards);
    ...
    static findFullHouse(cards) {
        ...
        rankCounts.sort((a, b) => {
            ...
            const cardA = new models_1.Card(types_1.Suit.SPADES, a.rank);
            const cardB = new models_1.Card(types_1.Suit.SPADES, b.rank);
        ...
        const pairRank = rankCounts.find((rc) => rc.count >= 2 && rc.rank !== threeOfAKindRank)?.rank;
        if (!pairRank) {
            return null;
        }
     ...
     */
    static evaluateHighHand(cards, aceHigh = true) {
        // Check for royal flush
        const royalFlush = this.findRoyalFlush(cards);
        if (royalFlush) {
            return {
                rank: types_1.HandRank.ROYAL_FLUSH,
                rankValue: 10,
        ...
        if (fourOfAKind.length > 0) {
            ...
            fourOfAKind.sort((a, b) => b[0].getValue(aceHigh) - a[0].getValue(aceHigh));
        ...
        if (threeOfAKinds.length > 0) {
            ...
            threeOfAKinds.sort((a, b) => b[0].getValue(aceHigh) - a[0].getValue(aceHigh));
        ...
        if (pairs.length >= 2) {
            ...
            // Find the highest kicker
            const kickers = cards.filter((card) => card.rank !== firstPair[0].rank && card.rank !== secondPair[0].rank);
            this.sortByRank(kickers, aceHigh);
            const kicker = kickers.length > 0 ? kickers[0] : null;
     ...
     */
    static evaluateLowHand(cards, qualifier = 8) {
        if (cards.length < 5) {
            return {
                valid: false,
                rankValue: 0,
     ...
     */
    static generateCombinations(cards, r = 5) {
        const combinations = [];
     ...
     */
    static generateOmahaCombinations(holeCards, communityCards) {
...
exports.HandUtils = HandUtils;
//# sourceMappingURL=HandUtils.js.map