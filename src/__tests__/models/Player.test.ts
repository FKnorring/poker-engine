...
import { PlayerStatus, PlayerPosition } from "../../types";
...
describe("Player", () => {
  ...
  beforeEach(() => {
    ...
    player = new Player("p1", "Player 1", 1000, 2);
  });
  ...
  test("should set and get position", () => {
    ...
    player.position = PlayerPosition.BUTTON;
    expect(player.position).toBe(PlayerPosition.BUTTON);

    player.position = PlayerPosition.SMALL_BLIND;
    expect(player.position).toBe(PlayerPosition.SMALL_BLIND);
